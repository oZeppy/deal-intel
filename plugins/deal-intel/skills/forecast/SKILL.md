...
4. **Draft** the message using `templates/forecast-message.md`, one block per chosen deal, in the order the rep gave. If the rep chose the sample format at step 0, use the sample layout already present in that file. Show the full draft.
...
- The "why" line is the rep's reasoning in their words when a deal room exists; otherwise it is a neutral fact (stage, next step), never invented sentiment.
